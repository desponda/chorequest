...
export function useParentData(): ParentData {
  ...
  const refetch = useCallback(async () => {
    ...
    try {
      const { data: profile, error: profileError } = await supabase.from('profiles').select('family_id').single()
      if (profileError || !profile) {
        setError('We could not load your family profile. Check your connection and try again.')
        return
      }
      ...
      if (familyRes.error || kidsRes.error || questsRes.error) {
        console.error('Failed to load parent workspace', familyRes.error ?? kidsRes.error ?? questsRes.error)
        setError('The command center could not load your family data. Check your connection and try again.')
        return
      }
      ...
      console.error('Failed to load parent workspace', loadError)
      setError('The command center could not load your family data. Check your connection and try again.')
    } finally {
      setLoading(false)
    }
  ...
  return {
    ...
    loading, error, supabase, refetch,
  }
}
